...
-- Создать цепочку неравенств:
-- x + y <= ... <= ... < 2
-- В более общем виде:
-- start (<=/</=) ... (<=/</=) ... (rel) finish

-- h1: позволяет перейти от 2х переменных к одной переменной (x)
-- h2: позволяет перейти от переменной (x) к конкретным числам
...
example {t : ℚ} (ht : t ≥ 10) : t ^ 2 - 3 * t - 17 ≥ 5 :=
  calc
    t ^ 2 - 3 * t - 17
      = t  * t - 3 * t  - 17 := by ring
 -- _ ≥ 10 * 10 - 3 * 10 - 17 := by rel [ht] -- error: t ≤ 10
 -- ...
    _ ≥ 5 := by sorry
...
example {n : ℤ} (hn : n ≥ 5) : n ^ 2 > 2 * n + 11 :=
  calc
    ...
    _ ≥ 5 * 5 := by rel [hn]
    _ > 2 * 5 + 11 := by numbers
    -- failed to synthesize `Trans` instance Trans GT.gt LE.le ?m.31335
    -- _ ≤ 2 * n + 11 := by sorry
    _ > 2 * n + 11 := by sorry
...
-- Новая тактика: extra
--
-- Доказывает штуки вида x + c ≥ x, при условии, что
-- у тебя из контекста понятно, что c ≥ 0.
--
-- С квадратами эта тема работает хорошо, т.е. для выражений вида
-- x + (a - b) ^ 2 ≥ x.
...
example {a b : ℚ} (h1 : a ≥ 3) (h2 : a + 2 * b ≥ 4) : a + b ≥ 3 :=
  calc
...
-- Показать, что левая + что-то >= правая
-- 2ab + C ≥ 2ab (by extra)

example (a b : ℝ) : a ^ 2 + b ^ 2 ≥ 2 * a * b :=
  calc
    a ^ 2 + b ^ 2 = 2 * a * b + (a ^ 2 + b ^ 2 - 2 * a * b) := by ring
