...
-- Несколько диких неудачных попыток.
...
-- 12.
-- Много неудачных странных попыток.
example {a b : ℝ} (h1 : a + 2 * b = 4) (h2 : a - b = 1) : a = 2 :=
  calc
    a = (a + 2 * b) - 2 * (a - b) + 2 * a - 4 * b := by ring
    _ = (a + 2 * b) - 2 * (a - b) + 2 * ((a - b) - b) := by ring
    _ = (a + 2 * b) - 2 * (a - b) + 2 * (1 - b) := by rw [h2]
    _ = (a + 2 * b) - 2 * a + 2 := by ring
    _ = (a + 2 * b) - 2 * (a - b) := by sorry
    _ = 4 - 2 * (a - b) := by rw [h1]
    _ = 4 - 2 * 1 := by rw [h2]
    _ = 2 := by ring
...
-- Если не получается подобрать выражения из комбинаций левых частей так,
-- чтобы можно было закэнслить переменные, то пробуй сложить правые и поделить на что-нибудь,
-- это ключевая стратегия в поиске доказательств для уравнений такого вида.
...
example {t : ℚ} (ht : t ^ 2 - 4 = 0) :
  ...
  calc
    t ^ 4 + 3 * t ^ 3 - 3 * t ^ 2 - 2 * t - 2 =
      (t ^ 2 - 4) ^ 2 + 3 * t ^ 3 + 5 * t ^ 2 - 2 * t - 18 := by ring
    _ = 0 ^ 2 + 3 * t ^ 3 + 5 * t ^ 2 - 2 * t - 18 := by rw [ht]
    _ = 3 * t * (t ^ 2 - 4) + 5 * (t ^ 2 - 4) + 10 * t + 2 := by ring
    _ = 3 * t * 0 + 5 * 0 + 10 * t + 2 := by rw [ht]
    _ = 10 * t + 2 := by ring
