...
-- Тактика cancel t может сократить на t только, если
-- Lean может вывести, что t ≠ 0. Здесь это следует из гипотезы h2,
-- которую мы не используем явно, но попробуй её убрать и увидишь ошибку.
...
example {a b : ℝ} (h1 : a ^ 2 = b ^ 2 + 1) (h2 : a ≥ 0) : a ≥ 1 := by
  have h3 :=
    calc
      ...
      _ ≥ 1 := by extra
      _ = 1 ^ 2 := by ring
  -- Кэнсэлим квадрат, т.е. делим на a, это требует чтобы a ≥ 0 (h2)
  cancel 2 at h3
...
example (a b : ℝ) (h1 : -b ≤ a) (h2 : a ≤ b) : a ^ 2 ≤ b ^ 2 := by
  ...
  calc
    -- Тк мы знаем, что: (b - a) * (b + a) ≥ 0,
    -- то по сути это имеет вид: x + z ≥ x
    -- Как раз работа для тактики extra.
...
example {x : ℚ} (h1 : x ^ 2 = 4) (h2 : 1 < x) : x = 2 := by
  have h3 :=
    calc
      ...
      _ = 4 + 2 * x := by rw [h1]
      _ = 2 * (x + 2) := by ring
  cancel (x + 2) at h3 -- гипотеза h2 нужна, чтобы это было возможно
...
example (x y : ℚ) (h : x * y = 1) (h2 : x ≥ 1) : y ≤ 1 := by
  have h0 : 0 < x * y := by addarith [h]
  have h1 : 0 < y := by cancel x at h0 -- using h2
  calc
    y ≤ x * y := by sorry
    _ = 1 := by rw [h]
